...
Deno.serve({ port: PORT }, async (req) => {
  ...
  try {
    ...
    if (err instanceof Deno.errors.NotFound) {
      console.warn(`⚠️  404: Không tìm thấy tệp: ${filepath} (Hãy chắc chắn bạn đã chạy 'deno task build' trước đó)`);
      return new Response("Not Found (Hãy chạy 'deno task build' trước khi truy cập)", { status: 404 });
    }
