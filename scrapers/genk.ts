...
export class GenKScraper implements Scraper {
  ...
  async fetchPosts(): Promise<Post[]> {
    ...
    for (let page = 1; page <= 3; page++) {
      ...
      try {
        ...
        if (!response.ok) {
          console.warn(
            `[GenK] Failed to fetch page ${page}: Status ${response.status}`,
          );
          continue;
        }
        ...
        $("li").each((_, element) => {
          ...
          const titleLink = $el.find(
            ".knswli-title a, .gkswli-title a, h3 a, a[href]",
          ).first();
          ...
          const summary = $el.find(
            ".knswli-right, p, .knswli-sapo, .gkswli-sapo, .sapo",
          ).first().text().trim();
  ...
  async fetchContent(url: string): Promise<string> {
    ...
    contentEl.find(
      "script, style, iframe, .ad-wrapper, .ad-position, .link-content-footer, .knc-relate-link, .cf-monitor",
    ).remove();
