...
export class HackerNewsScraper implements Scraper {
  ...
  async fetchPosts(): Promise<Post[]> {
    ...
    $(".body-post").each((index, element) => {
      ...
      const id = idMatch ? `thn-${idMatch[1]}` : `thn-${encodeURIComponent(postUrl).slice(-20)}`;

      // Trích xuất ngày đăng từ label: ví dụ "Jun 29, 2026"
      const labelText = $el.find(".item-label").text().trim();
      const dateMatch = labelText.match(/(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]* \d{1,2}, \d{4}/i);
  ...
  async fetchContent(url: string): Promise<string> {
    ...
    contentEl.find([
      "script", "style", "iframe",
      "meta", "link",
      ".ad-wrapper", ".cf-monitor", ".pop-under", ".adsbygoogle",
      ".post-head", ".schema_org",
      ".float-share", ".sharebelow", ".mobile-share",
      ".tags", ".note-b"
    ].join(", ")).remove();
