...
export class WindowsCentralScraper implements Scraper {
  ...
  async fetchPosts(): Promise<Post[]> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch Windows Central RSS: Status ${response.status}`,
      );
    }
    ...
    for (const item of items) {
      ...
      const author = $item.find("dc\\:creator, creator").text().trim() ||
        $item.find("dc:creator").text().trim() || "Windows Central";
  ...
  async fetchContent(url: string): Promise<string> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch Windows Central article: Status ${response.status}`,
      );
    }
    ...
    contentEl.find(
      "script, style, iframe, .ad-wrapper, .adsbygoogle, .pop-under, .social-share, .newsletter-signup, #utility-bar, .utility-bar",
    ).remove();
    // Loại bỏ phần quảng cáo reddit và hình ảnh đi kèm
    contentEl.find(
      "a[href*='reddit.com/r/windowscentral'], p:has(a[href*='reddit.com/r/windowscentral'])",
    ).remove();
